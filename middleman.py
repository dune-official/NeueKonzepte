...
def order():
    ...
    order_table.insert(description=json.get("description", ''), count=json["count"], file=json["file"],
    ...
    if bb_single["printer_status"] == 0:
        response = post(bb_single["location"] + "/print", json={"file": json["file"]})
...
def order_done(order_id):
    ...
    if done_already["count"] == exists["count"]:
